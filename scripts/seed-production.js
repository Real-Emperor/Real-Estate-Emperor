...
const prisma = new PrismaClient({ datasourceUrl: 'postgresql://neondb_owner:npg_wB1fOmTSF0ji@ep-still-sound-abnl454x.eu-west-2.aws.neon.tech/neondb?sslmode=require' });
const CID = 'company-1';
...
async function main() {
  ...
  const b1 = await prisma.property.create({ data:{id:'prop-arj-b1',companyId:CID,name:'Al Reef Al Junoobi - Building 1',nameAr:'الريف الجنوبي - المبنى 1',nameBn:'আল রিফ আল জুনুবি - ভবন ১',nameUr:'الریف الجنوبی - عمارت 1',type:'apartment',address:'Street 5, Khalifa City A, Abu Dhabi',totalUnits:15,floors:5} });
  const b2 = await prisma.property.create({ data:{id:'prop-arj-b2',companyId:CID,name:'Al Reef Al Junoobi - Building 2',nameAr:'الريف الجنوبي - المبنى 2',nameBn:'আল রিফ আল জুনুবি - ভবন ২',nameUr:'الریف الجنوبی - عمارت 2',type:'apartment',address:'Street 7, Khalifa City A, Abu Dhabi',totalUnits:14,floors:5} });
  const b3 = await prisma.property.create({ data:{id:'prop-ram-b1',companyId:CID,name:'Reef Al Madeena - Building 1',nameAr:'ريف المدينة - المبنى 1',nameBn:'রিফ আল মাদিনা - ভবন ১',nameUr:'ریف المدینہ - عمارت 1',type:'apartment',address:'Street 9, Khalifa City A, Abu Dhabi',totalUnits:16,floors:5} });
  const b4 = await prisma.property.create({ data:{id:'prop-ram-b2',companyId:CID,name:'Reef Al Madeena - Building 2',nameAr:'ريف المدينة - المبنى 2',nameBn:'রিফ আল মাদিনা - ভবন ২',nameUr:'ریف المدینہ - عمارت 2',type:'mixed_use',address:'Main Road, Musaffah, Abu Dhabi',totalUnits:10,floors:4} });
  ...
  const tds = [
    ...
    T('t-011','Habibur Rahman','حبيب الرحمن','হাবিবুর রহমান','حب الرحمن','050-217-6593','050-217-6593','784-1996-1122334-1','Bangladeshi','Al Reef Maintenance','107','studio',1,460,1400,'cash',2,70,b2.id,d(cy-1,1,1),d(cy,12,31),24,'active'),
    ...
    T('t-019','Kamal Hossain','كمال حسين','কমল হোসেন','کمال حسین','050-998-1122','050-998-1122','784-2001-9900112-9','Bangladeshi','Al Reef Maintenance','401','studio',4,440,1400,'cash',4,48,b2.id,d(cy-1,1,1),d(cy,12,31),24,'active'),
    ...
    T('t-028','Bishnu Prasad','بشنو براساد','বিষ্ণু প্রসাদ','وشنو پرساد','050-234-8899','050-234-8899','784-2002-3344556-3','Nepali','Al Reef Maintenance','401','studio',4,438,1500,'cash',0,88,b3.id,d(cy-1,1,1),d(cy,12,31),24,'notice'),
    ...
    T('t-031','Al Madina Grocery','بقالة المدينة','আল মদিনা মুদি দোকান','المدینہ گروسری','050-123-4567','050-123-4567','784-2000-1122334-1','Yemeni','Self-employed','Shop1','shop',1,500,4000,'cash',0,90,b4.id,d(cy-1,1,1),d(cy+1,12,31),36,'active'),
    ...
    T('t-033','Deepak Thapa','ديباك ثابا','দীপক থাপা','دیپک تھاپا','050-678-9900','050-678-9900','784-2003-4455667-4','Nepali','Al Reef Maintenance','104','studio',1,440,1400,'cash',0,86,b4.id,d(cy-1,1,1),d(cy,12,31),24,'active'),
  ...
  const exs=[
    ...
    {c:'manpower',d:'Maintenance staff - monthly',a:15000,v:'Al Reef Maintenance',i:'INV-2003',r:true,b:'All Buildings'},
    {c:'manpower',d:'Reception staff - monthly',a:6000,v:'Al Reef Maintenance',i:'INV-2005',r:true,b:'All Buildings'},
    ...
    {c:'maintenance',d:'AC repair B-108',a:380,v:'CoolTech Services',i:'INV-2010',r:false,b:'Al Reef Al Junoobi - Building 2'},
    {c:'maintenance',d:'Elevator maintenance - Al Reef Building 1',a:1800,v:'Schindler Elevators',i:'INV-2030',r:true,b:'Al Reef Al Junoobi - Building 1'},
    {c:'maintenance',d:'Painting - Hallway Al Reef Building 1',a:3200,v:'ColorPro Painters',i:'INV-2031',r:false,b:'Al Reef Al Junoobi - Building 1'},
    {c:'maintenance',d:'Plumbing repair - Reef Madeena Bldg 1',a:750,v:'Al Fix Plumbing',i:'INV-2040',r:false,b:'Reef Al Madeena - Building 1'},
    {c:'maintenance',d:'Elevator maintenance - Reef Madeena Bldg 1',a:1600,v:'Schindler Elevators',i:'INV-2032',r:true,b:'Reef Al Madeena - Building 1'},
    {c:'maintenance',d:'Elevator maintenance - Reef Madeena Bldg 2',a:1500,v:'Schindler Elevators',i:'INV-2033',r:true,b:'Reef Al Madeena - Building 2'},
    {c:'maintenance',d:'Elevator maintenance - Al Reef Building 2',a:1700,v:'Schindler Elevators',i:'INV-2034',r:true,b:'Al Reef Al Junoobi - Building 2'},
    {c:'leasing',d:'Leasing commission - 2 new tenants',a:4600,v:'Al Reef Leasing',i:'INV-2020',r:false,b:'All Buildings'},
    ...
    {c:'maintenance',d:'Roof waterproofing - Reef Madeena Bldg 2',a:8500,v:'WaterShield LLC',i:'INV-2050',r:false,b:'Reef Al Madeena - Building 2'},
    {c:'maintenance',d:'Parking lot repainting - Reef Madeena Bldg 2',a:2800,v:'ColorPro Painters',i:'INV-2051',r:false,b:'Reef Al Madeena - Building 2'},
  ...
  const mts=[
    ...
    {t:'Elevator Inspection - Al Reef Building 1',d:'Annual elevator inspection and certification.',c:'other',v:'Schindler Elevators',p:'medium',s:'pending',e:1500,a:null,pi:b1.id},
    {t:'Parking Lot Repainting - Reef Madeena Bldg 2',d:'Parking lines faded. Needs repainting.',c:'painting',v:'ColorPro Painters',p:'low',s:'completed',e:3000,a:2800,pi:b4.id},
    ...
    {t:'Intercom System Repair - Al Reef Building 2',d:'Intercom not working.',c:'electrical',v:'SafeWire Electric',p:'high',s:'in-progress',e:1800,a:null,pi:b2.id},
    {t:'Fire Extinguisher Replacement - Al Reef Building 1',d:'Annual fire extinguisher replacement.',c:'other',v:'FirePro Safety',p:'medium',s:'pending',e:900,a:null,pi:b1.id},
    ...
    {t:'AC Filter Cleaning - Reef Madeena Bldg 1',d:'Quarterly AC filter cleaning.',c:'ac',v:'CoolTech Services',p:'low',s:'completed',e:800,a:750,pi:b3.id},
    {t:'Staircase Lighting - Al Reef Building 2',d:'Staircase lights not working floors 3-5.',c:'electrical',v:'SafeWire Electric',p:'medium',s:'pending',e:400,a:null,pi:b2.id},
    {t:'Roof Waterproofing - Reef Madeena Bldg 2',d:'Water seepage on top floor.',c:'structural',v:'WaterShield LLC',p:'high',s:'pending',e:8000,a:null,pi:b4.id},
