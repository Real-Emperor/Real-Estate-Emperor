/**
 * Import TENANTS only - Properties already exist.
 * Uses individual inserts (no transactions) to avoid Neon timeout.
 */
...
// All occupied units from the Excel data
const OCCUPIED_UNITS = [
  ...
  { prop: 'Al Sinayaa Building', unit: 'SHOP1', tenant: 'ARSHAD BLDG MAT', rent: 0 },
  { prop: 'Al Sinayaa Building', unit: 'SHOP2', tenant: 'M.FAZLUL KARIM', rent: 2000 },
  { prop: 'Al Sinayaa Building', unit: 'SHOP3', tenant: 'SALOON', rent: 0 },
  { prop: 'Al Sinayaa Building', unit: 'SHOP4', tenant: 'AKHTER', rent: 0 },
  { prop: 'Al Sinayaa Building', unit: 'SHOP5', tenant: 'A/C CHANDAN', rent: 0 },
...
async function main() {
  console.log('=== IMPORTING TENANTS (no transactions) ===\n');
  
  // Load property ID map from DB
  const properties = await prisma.property.findMany({
    where: { companyId: COMPANY_ID, deletedAt: null },
    select: { id: true, name: true }
  });
  const propertyIdMap = new Map(properties.map(p => [p.name, p.id]));
  console.log(`Found ${properties.length} properties in DB\n`);
  
  // Check existing tenants
  const existingTenants = await prisma.tenant.count({ where: { companyId: COMPANY_ID, deletedAt: null } });
  if (existingTenants > 0) {
    console.log(`WARNING: ${existingTenants} tenants already exist. Clearing...`);
    const existing = await prisma.tenant.findMany({ where: { companyId: COMPANY_ID, deletedAt: null }, select: { id: true } });
    const ids = existing.map(t => t.id);
    if (ids.length > 0) {
      await prisma.receipt.deleteMany({ where: { tenantId: { in: ids } } });
      await prisma.payment.deleteMany({ where: { tenantId: { in: ids } } });
    }
    await prisma.tenant.updateMany({ where: { companyId: COMPANY_ID, deletedAt: null }, data: { deletedAt: new Date() } });
    console.log('Cleared.\n');
  }
  
  // Insert tenants one by one
  let created = 0;
  let errors = 0;
  const noRentTenants = [];
  
  for (let i = 0; i < OCCUPIED_UNITS.length; i++) {
    const row = OCCUPIED_UNITS[i];
    const propertyId = propertyIdMap.get(row.prop);
    if (!propertyId) {
      console.error(`  SKIP: No property "${row.prop}" in DB`);
      errors++;
      continue;
    }
    ...
    const notes = row.rent === 0 ? 'RENT AMOUNT PENDING - needs to be updated' : null;
    
    if (row.rent === 0) {
      noRentTenants.push(`${row.prop} - Unit ${row.unit} - ${row.tenant}`);
    }
    ...
    try {
      await prisma.tenant.create({
        data: {
          ...
          unitType: unitType,
          rentAmount: row.rent,
          municipalityFee: row.rent > 0 ? Math.round(row.rent * 0.05 * 100) / 100 : 0,
          securityDeposit: row.rent > 0 ? row.rent : null,
          status: 'active',
          notes: notes,
        }
      });
      ...
      console.error(`  ERROR: ${row.tenant} in ${row.prop}: ${err.message}`);
      errors++;
    }
    
    // Progress every 50
    if ((i + 1) % 50 === 0) {
      console.log(`  Progress: ${i + 1}/${OCCUPIED_UNITS.length} processed, ${created} created`);
    }
  }
  
  console.log(`\n=== TENANT IMPORT COMPLETE ===`);
  console.log(`Created: ${created}, Errors: ${errors}, Total: ${OCCUPIED_UNITS.length}`);
  
  if (noRentTenants.length > 0) {
    console.log(`\nWARNING: ${noRentTenants.length} tenants with rent=0 (needs update):`);
    noRentTenants.forEach(t => console.log(`  - ${t}`));
  }
  ...
  const props = await prisma.property.findMany({
    where: { companyId: COMPANY_ID, deletedAt: null },
    select: { name: true, type: true, totalUnits: true, _count: { select: { tenants: { where: { deletedAt: null } } } } },
    orderBy: { name: 'asc' }
  });
  
  let totalUnits = 0, totalTenants = 0;
  for (const p of props) {
    totalUnits += p.totalUnits;
    totalTenants += p._count.tenants;
    const vacant = p.totalUnits - p._count.tenants;
    console.log(`  ${p.name.padEnd(25)} | ${p.type.padEnd(10)} | Units: ${String(p.totalUnits).padStart(3)} | Tenants: ${String(p._count.tenants).padStart(3)} | Vacant: ${String(vacant).padStart(2)}`);
  }
  console.log(`\n  TOTAL: ${props.length} properties | ${totalUnits} units | ${totalTenants} tenants | ${totalUnits - totalTenants} vacant`);
  console.log(`  Occupancy: ${((totalTenants / totalUnits) * 100).toFixed(1)}%`);
